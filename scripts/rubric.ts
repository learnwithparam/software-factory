...
export const PHASES: Record<string, readonly Check[]> = {
	...
	'2 Design system': [
		...
		unit('tests/design.test.ts > every text, state and fill colour has a contrast pair', 1),
		unit('tests/design.test.ts > every generated stylesheet matches the token source', 2),
		unit('tests/design.test.ts > the yellow is a fill and never the colour of words', 1),
	],
	'3 Teach surfaces': [
		unit('tests/teach.test.ts > the guide carries every session exactly once, and the teaching folder is gone', 2),
		unit('tests/teach.test.ts > every concept the guide cites is declared by the workbook', 2),
		unit('tests/teach.test.ts > every concept the workbook declares is taught by a session', 2),
		unit('tests/teach.test.ts > every guide segment is complete', 1),
